...
client = MongoClient('localhost', 27017)  # mongoDB는 이주소(localhost)에 27017 포트로 돌아갑니다.
db = client.dbsparta  # 'dbsparta'라는 이름의 db를 만듭니다.

# MongoDB에 insert 하기

# db=기본적 저장 공간/ 여러 컬렉션 있음 / 'users'라는 collection에 데이터(딕셔너리)를 생성합니다. insert_one : 하나만 넣어라(생성해라)
